...
import { getMoviesDetails, getSimilarMovie } from "@/lib/api";
...
const MoviePopup = () => {
  ...
  useEffect(() => {
    const getMovie = async () => {
      try {
        setIsLoading(true);
        const extractMovieDetails = await getMoviesDetails(
          movie?.type,
          movie?.id
        );
        const similarMovies = await getSimilarMovie(movie?.type, movie?.id);
        ...
        {isLoading ? (
          <>
            ...
            <div className="grid mt-5 grid-cols-4 gap-3 items-center scrollbar-hide md:p-2">
              {[1, 2, 3, 4, 5, 6, 7, 8, 9, 19, 11, 12].map((_) => (
